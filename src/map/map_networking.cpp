...
void MapNetworking::finalizePacket(uint8* buff, size_t* buffsize, size_t PacketSize, MapSession* PSession, UsePreviousKey usePreviousKey)
{
    ...
    {
        ShowCritical("Network: compressed payload overflow before hash append (%zu) for char id %u", PacketSize, PSession->charID);
        *buffsize = 0;
        return;
    }
    ...
    {
        ShowCritical("Network: outgoing payload overflow (%zu > %u) for char id %u",
                     PacketSize,
